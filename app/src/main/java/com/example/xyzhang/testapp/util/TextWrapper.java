...
import com.example.xyzhang.testapp.HomeDisplayActivity;
...
public class TextWrapper {
    //    private static char[] endChars = {'s'};
    private static String endChars = "，。》、？；：’”】｝、！％）" + ",.>?;:]}!%)";

    public static void draw(int screenWidth, int screenHeight, String fontPath, Context context) throws IOException {
//        screenWidth = 650;
//        screenHeight = 1155;
...
//        float xPos = (screenWidth - lineWidth) / 2;//每一行的左边距
        ...
        while (nextCharIndex < total) {
            ...
            if (textChars[nextCharIndex]=='\n') {
                ...
                if (endChars.indexOf(textChars[nextCharIndex])>=0) {
                    nextCharIndex ++;
                }
                String line = text.substring(offset, nextCharIndex);
                if (lineIndex%linesEveryPage==1 && lineIndex > 1) {
...
//        for (int paraCount = 0; paraCount < paras.length; paraCount ++) {
//
//            String para = paras[paraCount];//当前段落的内容，字符串格式
//            char[] paraInChars = para.toCharArray();//当前段落的内容，字符数组格式
//            int paraLength = para.length();//每一段的字符个数
//
//            int offset = 0;//当前段落中已经编入行的字数
//
//            int lineWidth = 20 * metrics.charWidth('我');//每一行的标准宽度
//            int xPos = (screenWidth - lineWidth) / 2;//每一行的左边距
//
//            while (offset < paraLength) {//每次生成一行
//
//                int left = paraLength - offset;//还未编进行的字符数
//
//                int lineLength;//一行的字符个数
//                if (left <= 20) {
//                    lineLength = left;
//                } else {
//                    lineLength = 20;
//                    while (lineLength < paraLength && metrics.charsWidth(paraInChars, offset, lineLength) < lineWidth) {
//                        lineLength ++;
//                    }
//                }
//
//                if (offset+lineLength<para.length() && endChars.indexOf(para.charAt(offset+lineLength))>=0) {
//                    lineLength ++;
//                }
//                String line = para.substring(offset, offset+lineLength);
//
//                g.drawString(line, xPos, lineIndex * lineHeight);
//
//                offset += lineLength;
//                if ()
//
